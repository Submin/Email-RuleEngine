...
TestHeader13: {"Addr":["192.168.1.13","192.168.2.13"]}
TestHeader14: {"Addr":["192.168.1.13","192.168.4.13"]}
...
describe "Email::RuleEngine::Condition::Subnet success on ALL Header with" => sub {
    before all => sub {
    ...
    it "single address value is partially match in conf subnets list" => sub {
        is $cond->run("in", ( 'Header/TestHeader11', 'Subnet/["192.168.1.0/24","192.168.2.0/24"]' )), 0;
    };
    it "single address value is full not match in conf subnets list" => sub {
    ...
    it "multigle addresses value as JSON string partially is match in conf subnets list" => sub {
        is $cond->run("in", ( 'Header/TestHeader14', 'Subnet/["192.168.1.0/24","192.168.2.0/24"]' )), 0;
    };
    it "multigle addresses value as JSON string full is not match in conf subnets list" => sub {
